...
/**
 * Extracts data from xaips and writes them as files to specified output folder. Writes verification reports
 * to outputfolder.
 */
public class DumpHandler
{
  ...
  private void dumpDataSection(XaipAndSerializer xas, OutputFolder outFolder)
  {
    ...
    var xaipObjectWriter = new XaipObjectWriter().withOutputFolder(outFolder)
                                                 .withXaipSerializer(xas.getSerializer());
  ...
  private void dumpSignature(XaipAndSerializer xas, OutputFolder outFolder) throws IOException
  {
    ...
    for ( var cred : credentialSection.getCredential() )
    {
      ...
      {
        LOG.warn("Credential {} is not a supported signature object. Supported are: http://www.w3.org/2000/09/xmldsig#Signature, dss:Timestamp and dss:Base64Signature",
