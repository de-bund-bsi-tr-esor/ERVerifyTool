...
public class TestECardDetachedSignatureValidator
{
  ...
  /**
   * Checks several get methods, assert that an unreachable eCard service produces an internal error with
   * apprehensive message.
   */
  @Test
  public void notReachable() throws Exception
  {
    Configurator.getInstance()
                .getProfile("custom")
                .setValidationService("http://not-reachable:1234/eCardService");
    ...
    assertThat(report.getOverallResult()
                     .getResultMessage()
                     .getValue()).startsWith("eCard webservice is unreachable");
    assertThat(report.getVr().getIndividualReport()).hasSize(1);
    assertThat(report.getVr()
                     .getIndividualReport()
                     .get(0)
                     .getResult()
                     .getResultMessage()
                     .getValue()).contains("eCard webservice is unreachable");
  }

  /**
   * Checks that {@link SignatureReportPart} returns all the needed values. One test for report part does not
   * justify whole new class.
   */
  @Test
  public void checkReportPart()
  {
    ...
    var elem = new JAXBElement<>(new QName("DetailedSignatureReport"), DetailedSignatureReportType.class,
                                 new DetailedSignatureReportType());
  ...
  public void generateValidationMessageOptionalOutputsEmpty() throws IOException
  {
    ...
    assertThat(message).isEqualTo("Illegal eCard response. The optional outputs section that was received from the eCardService is empty.");
  }
  ...
  /**
   * Asserts that validation message is generated because of an invalid result in SigMathOk and restricted
   * validation mode.
   */
  @Test
  ...
  /**
   * For debugging: writes requests into a fixed file. That need arises because there are many ways to include
   * data into a VerifyRequest and it is likely that any service implementation supports not all of them.
   */
  ...
  private static IndividualReportType createIndividualReport(ValidationResultMajor sigMathOkResult)
  {
    ...
    var detailedSignatureReportJaxb = new JAXBElement<>(new QName("dummy"), DetailedSignatureReportType.class,
