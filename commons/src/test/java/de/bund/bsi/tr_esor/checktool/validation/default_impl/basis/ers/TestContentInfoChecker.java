...
public class TestContentInfoChecker
{
    ...
    /**
     * Tests the validator with a mocked ContentInfo element that conforms to Basis-ERS-Profil. Assert that the result is valid.
     *
     * @throws Exception
     */
    @Test
    ...
    /**
     * Tests the validator with mocked ContentInfo element that violates the Basis-ERS-Profil in the most possible way. Assert all expected
     * error messages are present and the result is invalid.
     *
     * @throws Exception
     */
    @Test
    public void testCheckSignedDataInvalid() throws Exception
    {
        ...
        var msgs = new String[]{"contentType OID of time stamp is not " + ContentInfoChecker.OID_PKCS7_SIGNEDDATA,
            ...
            "digestAlgorithms must be filled",
            "encapContentInfo must be filled",
            ...
            "signatureAlgorithm must be present",
            "signatureValue must be present",
            "unsignedAttrs must be omitted",
            "attributes must be DER encoded",
    ...
    /**
     * Tests the validator with mocked ContentInfo element with alternative violations of the Basis-ERS-Profil. Assert all expected error
     * messages are present and the result is invalid.
     *
     * @throws Exception
     */
    @Test
    ...
    /**
     * Tests a violation of Basis-ERS-Profil in content info, assert an invalid result and the appropriate error message.
     *
     * @throws Exception
     */
    @Test
    public void testCheckContentInfo() throws Exception
    {
        ...
        assertThat(formatOk.getSummarizedMessage(), containsString("Could not parse element, error was: unknown object"));
    }

    /**
     * Tests a violation of Basis-ERS-Profil in signed data, assert an invalid result and the appropriate error message.
     *
     * @throws Exception
     */
    @Test
    ...
    /**
     * Tests a violation of Basis-ERS-Profil in RevocationInfoChoices, assert an invalid result and the appropriate error message.
     *
     * @throws Exception
     */
    @Test
    ...
    /**
     * Tests a violation of Basis-ERS-Profil in ContentType, assert an invalid result and the appropriate error message.
     *
     * @throws Exception
     */
    @Test
    ...
    /**
     * Tests some violations of Basis-ERS-Profil in SigningCertificateV2, assert an invalid result and the appropriate error message.
     *
     * @throws Exception
     */
    @Test
    public void testCheckSigningCertificateAttribute() throws Exception
    {
        ...
        var notScV2Attribute = new Attribute(new ASN1ObjectIdentifier("1.2.3"), new BERSet(new DERUTF8String("dummy")));
    ...
    private ContentInfo mockContentInfo(boolean valid, boolean wrongElement) throws Exception
    {
        ...
        when(ci.getContentType()).thenReturn(new ASN1ObjectIdentifier(valid
            ? ContentInfoChecker.OID_PKCS7_SIGNEDDATA
            : "1.2.840.113549.3.2"));
        return ci;
    }
    ...
    private ContentInfo mockEncapContenInfo(boolean wrongElement)
    {
        ...
        when(encapContentInfo.getContentType()).thenReturn(new ASN1ObjectIdentifier(wrongElement
            ? ContentInfoChecker.OID_PKCS7_SIGNEDDATA
            : ContentInfoChecker.OID_TST_INFO));
    ...
    private ASN1TaggedObject mockRevocationFormat(boolean valid, boolean wrongElement)
    {
        ...
        otherRevVector.add(new ASN1ObjectIdentifier(wrongElement ? "1.3.3.7.1" : ContentInfoChecker.OID_BASIC_OSCP_RESPONSE));
    ...
    private BasicOCSPResponse mockOCSPResponse(boolean valid)
    {
        ...
        ResponderID respID = when(mock(ResponderID.class).getName()).thenReturn(valid ? new X500Name("CN=test") : null).getMock();
        ResponseData respData = when(mock(ResponseData.class).getResponderID()).thenReturn(respID).getMock();
        Extensions singleExt =
            when(mock(Extensions.class).getExtension(ArgumentMatchers.any())).thenReturn(valid ? mock(Extension.class) : null).getMock();
        SingleResponse singleResp = when(mock(SingleResponse.class).getSingleExtensions()).thenReturn(singleExt).getMock();
    ...
    private ASN1Set getSignerInfoSet(boolean valid, boolean wrongElement) throws Exception
    {
        ...
        SignerIdentifier sid =
            when(mock(SignerIdentifier.class).getId()).thenReturn(valid ? mock(IssuerAndSerialNumber.class) : null).getMock();
        when(si.getSID()).thenReturn(sid);
        when(si.getDigestAlgorithm()).thenReturn(valid ? new AlgorithmIdentifier(new ASN1ObjectIdentifier("1.2.3")) : null);
        ...
        when(si.getUnauthenticatedAttributes()).thenReturn(valid ? null : new BERSet(new DERUTF8String("Bad attribute")));
    ...
    private ASN1Set mockSignedAttributes(boolean valid) throws Exception
    {
        ...
        SigningCertificateV2 scv2 = when(mock(SigningCertificateV2.class).getCerts()).thenReturn(new ESSCertIDv2[valid ? 1 : 0]).getMock();
        saVector.add(new Attribute(new ASN1ObjectIdentifier(ContentInfoChecker.OID_SIGNING_CERTIFICATTE_V2), new BERSet(scv2)));
        if (!valid)
        {
