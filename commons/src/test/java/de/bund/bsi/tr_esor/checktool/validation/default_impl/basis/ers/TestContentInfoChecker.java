...
public class TestContentInfoChecker
{
  ...
  public void testCheckSignedDataInvalid() throws Exception
  {
    FormatOkReport formatOk = new FormatOkReport(ref);
    ContentInfoChecker systemUnderTest = new ContentInfoChecker(formatOk);
    systemUnderTest.checkContentInfo(ref, mockValidContentInfo(false, false));
    assertThat(formatOk.getOverallResult().getResultMajor(), endsWith(":invalid"));
    String[] msgs = new String[]{"contentType OID of time stamp is not "
                                 + ContentInfoChecker.OID_PKCS7_SIGNEDDATA, "version must be 3",
  ...
  public void testCheckSignedDataWrongFormat() throws Exception
  {
    FormatOkReport formatOk = new FormatOkReport(ref);
    ContentInfoChecker systemUnderTest = new ContentInfoChecker(formatOk);
    systemUnderTest.checkContentInfo(ref, mockValidContentInfo(true, true));
    assertThat(formatOk.getOverallResult().getResultMajor(), endsWith(":invalid"));
  ...
  private ContentInfo mockValidContentInfo(boolean valid, boolean wrongElement) throws Exception
  {
    ContentInfo ci = mock(ContentInfo.class);
    SignedData sd = mockValidSignedData(valid, wrongElement);
  ...
  private SignedData mockValidSignedData(boolean valid, boolean wrongElement) throws Exception
  {
    ...
    ASN1Set siSet = getSignerInfoSet(valid, wrongElement);
  ...
  private ASN1Set getSignerInfoSet(boolean valid, boolean wrongElement) throws Exception
  {
    ...
    ASN1Set signedAttributes = mockSignedAttributes(valid);
