...
public class TestDummyTimeStampValidator
{
  ...
  private static byte[] correctContent = Base64.decode("Bgp7V7c8oOwxO0B6YxslVn3BepntQwRfBNBK1ERQ8qZPyCsmrstH0oaMTvvjWBcyo+fLzGwu+zIGLAgXCgXuuIFNeJYrD4rCvWPa+fAT7QwH/mf7+/vBUrMKR2MEoFNduXbqspOmCKCbE6zPVwtPoif/rzoQ0wauN4JIRm0Ff+M=");
  ...
  public void detectsTimestampHashNotMatchingData() throws IOException
  {
    ...
    assertThat(report.getFormatted()
                     ...
                     .getSigMathOK()
                     .getResultMessage()
                     .getValue()).contains("does not match the calculated root hash value of the partial hashtree");
  }
  ...
  public void detectsTimestampHashNotMatchingSignature() throws Exception
  {
    ...
    var hashValue = new LocalHashCreator().calculateHash(hashedContent,
    ...
    var report = sut.validateInternal(new Reference("ignored"),
                                      new TimeStampToken(new CMSSignedData(encoded)));
    assertThat(report.getFormatted()
                     .getSignatureOK()
                     .getSigMathOK()
                     .getResultMajor()).isEqualTo(ValidationResultMajor.INVALID.toString());
    assertThat(report.getFormatted()
                     .getSignatureOK()
                     .getSigMathOK()
                     .getResultMinor()).isEqualTo(ECardResultMinor.INVALID_SIGNATURE);
    assertThat(report.getFormatted()
                     .getSignatureOK()
                     .getSigMathOK()
                     .getResultMessage()
                     .getValue()).contains("Validation of the mathematical correctness of the given timestamp failed");
  }
