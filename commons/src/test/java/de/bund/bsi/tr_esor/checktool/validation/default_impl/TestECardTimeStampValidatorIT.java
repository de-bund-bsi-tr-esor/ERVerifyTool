...
/**
 * Tests the online validation capabilities for timestamps using the eCard. This isolated test cannot reproduce the actual behaviour, where
 * the timestamp's hash source is determined when all validators work together. This is why we here can test for indetermined at best for a
 * valid timestamp. Other tests ensure online validation can result in valid results.
 */
public class TestECardTimeStampValidatorIT
{
    ...
    public void detectsHashValueMismatch() throws Exception
    {
        TimeStampReport report =
            validateErFromRessourcesForContent("/xaip/xaip_ok.ers.b64", "custom", "mismatch".getBytes(StandardCharsets.UTF_8));
        ...
        assertThat(report.getFormatted().getSignatureOK().getSigMathOK().getResultMajor(), containsString("invalid"));
    }

    /**
     * Asserts that a test time stamp is checked as invalid if the TR-ESOR profile requiring qualified timestamps is selected
     */
    @Test
    ...
    private TimeStampReport validateErFromRessourcesForContent(String testResource, String profileName, byte[] content) throws IOException
    {
    ...
    /**
     * Asserts that a qualified time stamp is checked as indetermined if the TR-ESOR profile requiring qualified timestamps is selected, but
     * no data is given. Also asserts quality information is added to the report.
     */
    @Test
