...
public class TestECardResponseUtil
{
  ...
  /**
   * This test case is based the answer provided by the verification interpreter web service if no signature
   * is detected.
   */
  @Test
  public void detectsMissingSignatureFromOKandNoReport()
  {
    ...
    optionalOutputs.getAny()
                   .add(FACTORY_OASIS_VR.createVerificationReport(FACTORY_OASIS_VR.createVerificationReportType()));
    ResponseBaseType response = buildResponse(ECardResultMajor.OK, null, null, optionalOutputs);
    assertTrue("OK without report is an acceptable eCard result.",
               ECardResponseUtil.isAcceptableECardResult(response));
    assertTrue("OK without report is detected as no signature found.",
               ECardResponseUtil.isNoSignatureFound(response));
  }

  /**
   * This test case is based the answer provided by the crypto service library web service if there is no
   * plugin that detected a signature in the data provided.
   */
  @Test
  public void detectsMissingSignatureFromErrorNoPluginFound()
  {
    ResponseBaseType response = buildResponse(ECardResultMajor.ERROR,
                                              ...
                                              ECardResultMessage.RESULTMESSAGE_NO_PLUGIN,
                                              null);
    assertFalse("No plugin found is detected as acceptable result.",
                ECardResponseUtil.isAcceptableECardResult(response));
    assertTrue("No plugin found is detected as no signature found.",
               ECardResponseUtil.isNoSignatureFound(response));
  }

  /**
   * This test case is represents the answer received from the crypto service library in case an unsigned XML
   * or PDF was detected.
   */
  @Test
  ...
  {
    ResponseBaseType response = buildResponse(ECardResultMajor.ERROR,
                                              ECardResultMinor.NO_PERMISSION,
                                              null,
                                              null);
    assertFalse("No permission is detected as technical error.",
                ECardResponseUtil.isAcceptableECardResult(response));
    assertFalse("No permission is not detected as no signature found.",
                ECardResponseUtil.isNoSignatureFound(response));
  }
  ...
  public void errorsOnMissingECardMinor()
  {
    ...
    assertFalse("An error without message is detected as unacceptable result.",
                ECardResponseUtil.isAcceptableECardResult(response));
    assertFalse("An error without message is not detected as no signature found.",
                ECardResponseUtil.isNoSignatureFound(response));
  }
  ...
  public void errorsOnBadECardMinor()
  {
    ...
    assertFalse("An unknown minor code is detected as unacceptable result.",
                ECardResponseUtil.isAcceptableECardResult(response));
    assertFalse("An unknown minor code is not detected no signature found.",
                ECardResponseUtil.isNoSignatureFound(response));
  }
  ...
  {
    ResponseBaseType response = buildResponse(ECardResultMajor.WARNING,
                                              ECardResultMinor.DETACHED_SIGNATURE_WITHOUT_E_CONTENT,
                                              null,
                                              null);
    assertTrue("A result for missing eContent is detected as acceptable ecard result.",
               ECardResponseUtil.isAcceptableECardResult(response));
    assertFalse("A result for missing eContent is not detected no signature found.",
                ECardResponseUtil.isNoSignatureFound(response));
  }
  ...
  {
    ResponseBaseType response = buildResponse(ECardResultMajor.ERROR,
                                              ECardResultMinor.WRONG_MESSAGE_DIGEST,
                                              null,
                                              null);
  ...
  private ResponseBaseType buildResponse(String resultMajor,
                                         String resultMinor,
                                         String resultMessage,
                                         AnyType optionalOutputs)
  {
