...
/**
 * Checks that evidence records are located properly in the input data and verification is done for the correct set of data. This test
 * addresses the input preparation as well as the validation itself.
 */
public class TestErValidation
{
    ...
    /**
     * Asserts that a report only for an evidence record itself can be created in case the protected data itself is not available. This
     * feature is not required by Feinkonzept.
     * <p>
     * In case protected data objects are given, any hash mismatch will make the ER report invalid. That is tested by the respective entry
     * tests, for instance {@link TestS4VerifyOnly}.
     */
    @Test
    ...
    /**
     * Asserts that the report contains a meaningful report if the request contains a profile name which is not supported.
     */
    @Test
    ...
    public void brokenIntegrityInLXaipYieldsToValueMismatchError() throws Exception
    {
        ...
        assertThat(result.getResultMessage().getValue(),
            containsString(
                "The calculated digest value of the LXAIP data object (id: HundesteuerAnmeldung_V001) does not match the embedded digest"));
    ...
    public void manipulatedXaip() throws Exception
    {
        ...
        binary.setValue(new DataHandler(new ByteArrayDataSource("FAKEEEE".getBytes(StandardCharsets.UTF_8), "text/plain")));
        params.setXaip("/xaip/xaip_ok_ers.xml");
    ...
    public void missingDataFromXaip() throws Exception
    {
        ...
        var protectedObjects =
            params.getXaip().getPackageHeader().getVersionManifest().get(0).getPackageInfoUnit().get(0).getProtectedObjectPointer();
        var versionManifest = protectedObjects.stream().filter(o -> o.getValue() instanceof VersionManifestType).findFirst();
        var metaData = protectedObjects.stream().filter(o -> o.getValue() instanceof MetaDataObjectType).findFirst();
        ...
        assertThat(result.getResultMessage().getValue(),
            allOf(containsString("additional protected hash values"),
                containsString(
                    "Additional hashes:[814d78962b0f8ac2bd63daf9f013ed0c07fe67fbfbfbc152b30a476304a0535d, b976eab293a608a09b13accf570b4fa227ffaf3a10d306ae378248466d057fe3]"),
                containsString(
                    "Expected hashes for: [XAIP_TEST_VALUE/metaDataID:fileSize_V001, XAIP_TEST_VALUE/dataObjectID:HundesteuerAnmeldung_V001]")));
    }

    /**
     * Asserts that an evidence record referring to no version works as long that there is only one version in the xaip
     */
    @Test
    ...
    /**
     * Asserts that several evidence records in one XAIP can be validated and are assigned to the respective version.
     */
    @Test
    ...
    /**
     * Asserts that a XAIP containing two version manifests and an ER not pointing to any version can not be validated.
     */
    @Test
    public void testNoVersionDefinedInErWithMoreThanOneVersionManifest() throws Exception
    {
        ...
        assertThat(result.getResultMessage().getValue(),
            containsString(
                "There is more than one VersionManifest in the Xaip. The EvidenceRecord needs to specify which version it relates to."));
    }

    /**
     * Asserts that a XAIP containing reference to the version manifest for the credential enveloping an evidence record can be validated
     * even if no version ID is set for the evidence record.
     */
    @Test
    ...
    /**
     * Asserts that a XAIP containing reference to two version manifest for the credential enveloping an evidence record can not be
     * validated.
     */
    @Test
    public void testErVersionInRelatedObjectsPointsToTwoVersionManifests() throws Exception
    {
        ...
        assertThat(result.getResultMajor(), endsWith(":RequesterError"));
        assertThat(result.getResultMinor(), containsString("parameterError"));
        assertThat(result.getResultMessage().getValue(), containsString("An EvidenceRecord can only refer to one VersionManifest."));
    }

    /**
     * Asserts that a XAIP containing reference to a dataObject instead of a version manifest for the credential enveloping an evidence
     * record can not be validated.
     */
    @Test
    ...
    /**
     * Asserts that a XAIP containing a reference to the version manifest for the credential enveloping an evidence record can be
     * validated.
     */
    @Test
    ...
    /**
     * Asserts that a XAIP containing a mismatching reference to the version manifest for the credential enveloping an evidence record is
     * rejected.
     */
    @Test
    ...
    private VerificationReportType validate(ParameterFinder params) throws ReflectiveOperationException, IOException
    {
