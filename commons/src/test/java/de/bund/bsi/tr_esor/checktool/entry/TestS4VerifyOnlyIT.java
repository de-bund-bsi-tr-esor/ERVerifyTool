...
/**
 * Integration Test (needs running crypto server and verification endpoint) for the VerifyRequest defined by TR-ESOR S.4
 */
...
public class TestS4VerifyOnlyIT
{
    ...
    public void verifiesLXaipWithCredAndER() throws Exception
    {
        ...
        assertThat(response.body(),
            containsString(
                "SignedObjectIdentifier XPath=\"VerifyRequest/InputDocuments/Document[@id='g']/InlineXML/credentialSection/credential[@credentialID='ER_2.16.840.1.101.3.4.2.1_V001']/evidenceRecord/asn1EvidenceRecord\"/>"));
    ...
    public void verifiesLXaipWithMetaAndER() throws Exception
    {
        ...
        assertThat(response.body(),
            containsString(
                "SignedObjectIdentifier XPath=\"VerifyRequest/InputDocuments/Document[@id='g']/InlineXML/credentialSection/credential[@credentialID='ER_2.16.840.1.101.3.4.2.1_V001']/evidenceRecord/asn1EvidenceRecord\"/>"));
        assertThat(response.body(), containsString("SignedObjectIdentifier FieldName=\"HundesteuerAnmeldung_V001\""));
        assertThat(response.body(), not(containsString(PARAMETER_ERROR)));
        assertThat(response.body(), not(containsString(MAJOR_INVALID)));
        assertThat(response.body(), not(containsString(MAJOR_INDETERMINED)));
    }
    ...
    {
        var request =
            HttpRequest.newBuilder().uri(URI.create(ENDPOINT_URL)).POST(HttpRequest.BodyPublishers.ofFile(Path.of(requestFile))).build();
