...
public class LXaipReader
{
    ...
    public byte[] readBinaryData(Object data, String id)
    {
            ...
            InputStream ins = new BufferedInputStream(fi))
        {
            ...
            var digestCorrect = isDigestCorrect(binaryData, dataObjectReference.getDigestMethod(), dataObjectReference.getDigestValue());
            if (!digestCorrect)
            {
                throw new LXaipDigestMismatchException(String.format(
                    "The calculated digest value of the LXAIP data object (id: %s) does not match the embedded digest",
                    id), id);
            }
        ...
        {
            throw new LXaipUnprocessableException(String.format(
                "Cannot read LXAIP's data object (id: %s) from file %s. Adjust lxaipDataDirectory configuration and/or the LXAIP uri accordingly",
                id,
                path.toAbsolutePath()), id, e);
        }
    ...
    private static void ensureNoDirectoryTraversal(Path directory, Path fileWithin, String dataObjectId)
    {
        try
        {
            ...
            {
                throw new LXaipUnprocessableException(String.format(
                    "LXAIP data object (id: %s) reference uri is not allowed. Avoid using '..'. Not allowed was: %s",
                    dataObjectId,
                    fileWithin), dataObjectId);
            }
        }
        catch (IOException e)
        {
            throw new LXaipUnprocessableException(String.format("LXAIP data object (id: %s) reference uri cannot be resolved. (%s)",
                dataObjectId,
                fileWithin), dataObjectId, e);
        }
    }

    private boolean isDigestCorrect(byte[] binaryData, DigestMethodType digestMethod, byte[] digestValue) throws NoSuchAlgorithmException
    {
    ...
    private static boolean isLXaip(Object data)
    {
        ...
        return xmlData != null
            && xmlData.size() == 1
            && xmlData.get(0) != null
            && xmlData.get(0) instanceof JAXBElement
            && ((JAXBElement)xmlData.get(0)).getValue() != null
    ...
    private static DataObjectReferenceType dataObjectReferenceFrom(Object data)
    {
        if (data instanceof DataObjectType)
        {
            return (DataObjectReferenceType)((JAXBElement)((DataObjectType)data).getXmlData().getAny().get(0)).getValue();
        }
        if (data instanceof MetaDataObjectType)
        {
            return (DataObjectReferenceType)((JAXBElement)((MetaDataObjectType)data).getXmlMetaData().getAny().get(0)).getValue();
        }
        if (data instanceof CredentialType)
        {
            return (DataObjectReferenceType)((JAXBElement)((CredentialType)data).getOther().getAny().get(0)).getValue();
        }
        return null;
    }
