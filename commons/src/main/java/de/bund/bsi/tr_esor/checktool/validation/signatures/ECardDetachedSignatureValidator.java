...
/**
 * Validator for signatures. It issues an eCard VerifyRequest to the configured eCard-compliant web service and adds the returned
 * VerificationReport to the context.
 *
 * @author TT, WS
 */
public class ECardDetachedSignatureValidator extends BaseECardSignatureValidator<SignatureObject, DetachedSignatureValidationContext>
{
    ...
    protected VerifyRequest createVerifyRequest(SignatureObject sig) throws JAXBException
    {
        ...
        {
            DocumentType doc = createBase64Document(entry.getKey().relativize(ctx.getReference()), entry.getValue());
    ...
    protected boolean isRestrictedValidation(VerificationReportType verificationReport, SignatureObject toCheck)
    {
        if (ctx.isRestrictedValidation() && toCheck != null && toCheck.getSignature() != null)
        {
            ...
            return verificationReport.getIndividualReport()
                ...
                .anyMatch(sigMathOk -> ValidationResultMajor.INVALID.toString().equals(sigMathOk.getResultMajor()));
        }
        return false;
    }
