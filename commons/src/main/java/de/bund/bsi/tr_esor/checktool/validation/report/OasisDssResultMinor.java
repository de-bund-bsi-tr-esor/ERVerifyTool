...
/**
 * Possible minor result values for element {@code<Result>} from {@code urn:oasis:names:tc:dss:1.0:core:schema }.
 * <p>
...
public enum OasisDssResultMinor
{
    ...
    /**
     * The signature or timestamp is valid. Furthermore, the signature or timestamp covers all of the input documents just as they were
     * passed in by the client.
     */
    SUCCESS_VALID_FOR_ALL_DOCUMENTS(OasisDssResultMajor.SUCCESS, "urn:oasis:names:tc:dss:1.0:resultminor:valid:signature:OnAllDocuments"),

    /**
     * The signature or timestamp is valid. However, the signature or timestamp does not cover all of the input documents that were passed
     * in by the client.
     */
     ...
     */
    SUCCESS_INVALID_SIGNATURE(OasisDssResultMajor.SUCCESS, "urn:oasis:names:tc:dss:1.0:resultminor:invalid:IncorrectSignature"),

    /**
     * The signature is valid with respect to XML Signature core validation. In addition, the message also contains
     * VerifyManifestResults.<br> Note: In the case that the core signature validation failed no attempt is made to verify the manifest.
     */
    ...
    /**
     * A ds:Reference element is present in the ds:Signature containing a full URI, but the corresponding input document is not present in
     * the request.
     */
     ...
     */
    ERROR_REQUEST_KEY_INFO_NOT_PROVIDED(OasisDssResultMajor.REQUESTER_ERROR, "urn:oasis:names:tc:dss:1.0:resultminor:KeyInfoNotProvided"),
     ...
     */
    ERROR_REQUEST_INVALID_REF_URI(OasisDssResultMajor.REQUESTER_ERROR, "urn:oasis:names:tc:dss:1.0:resultminor:InvalidRefURI"),

    /**
     * The server was not able to parse a Document.
     */
    ERROR_REQUEST_XML_NOT_PARSEABLE(OasisDssResultMajor.REQUESTER_ERROR, "urn:oasis:names:tc:dss:1.0:resultminor:NotParseableXMLDocument"),

    /**
     * The server doesn’t recognize or can’t handle any optional input.
     */
    ERROR_REQUEST_NOT_SUPPORTED(OasisDssResultMajor.REQUESTER_ERROR, "urn:oasis:names:tc:dss:1.0:resultminor:NotSupported"),

    /**
     * The signature or its contents are not appropriate in the current context. For example, the signature may be associated with a
     * signature policy and semantics which the DSS server considers unsatisfactory.
     */
    ...
    /**
     * The processing of the request failed due to an error not covered by the existing error codes. Further details should be given in the
     * result message for the user which may be passed on to the relevant administrator.
     */
    ERROR_RESPONSE_GENERAL_ERROR(OasisDssResultMajor.RESPONDER_ERROR, "urn:oasis:names:tc:dss:1.0:resultminor:GeneralError"),

    /**
     * Locating the identified key failed (e.g. look up failed in directory or in local key file).
     */
    ERROR_RESPONSE_KEY_LOOKUP_FAILED(OasisDssResultMajor.RESPONDER_ERROR, "urn:oasis:names:tc:dss:1.0:resultminor:invalid:KeyLookupFailed"),
     ...
     */
    INSUFFICIENT_CRL_NOT_AVAILABLE(OasisDssResultMajor.INSUFFICIENT_INFORMATION, "urn:oasis:names:tc:dss:1.0:resultminor:CrlNotAvailiable"),
    ...
    /**
     * The chain of trust could not be established binding the public key used for validation to a trusted root certification authority via
     * potential intermediate certification authorities.
     */
