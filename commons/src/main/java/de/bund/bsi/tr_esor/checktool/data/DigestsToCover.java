...
/**
 * Class for storing lists of digests that should be covered by a specific timestamp and appropriate
 * configuration. Supports that a different hash mode (sorted/unsorted) can be inserted to check.
 */
public class DigestsToCover
{
  ...
  /**
   * Constructor for use cases where there is no alternative of sorted and unsorted hashes, i.e. all cases
   * except for the first timestamp of an ArchiveTimestampChain produced by a rehash.
   *
   * @param hashes Map of the references and the hashes generated for them
   * @param checkForAdditionalHashes set to true to expect that no additional hashes not given by hashes are
  ...
  /**
   * Constructor for use cases where there is an alternative of sorted and unsorted hashes, i.e. only where
   * the first timestamp of an ArchiveTimestampChain produced by a rehash.
   *
   * @param sortedHashes Map of the references and the hashes generated for them if hashes are ordered before
   *          generating the root hash
   * @param unsortedHashes Map of the references and the hashes generated for them if hashes are generated
   *          according to the RFC
   * @param checkForAdditionalHashes set to true to expect that no additional hashes not given by hashes are
   *          covered by the evidence record.
   * @param expectSortedHashes set to true to expect the sorted hashes to be contained in the ER
   */
  public DigestsToCover(Map<Reference, byte[]> sortedHashes,
                        Map<Reference, byte[]> unsortedHashes,
  ...
  /**
   * Get the hash sorting mode expected for this DigestsToCover. Might be null if no alternative hashes are
   * present or any of the HashSortingMode enum values.
   */
  ...
  /**
   * Stream entries consisting of References and the hash values for the alternative hash mode. This will only
   * work if alternative hashes are present.
   */
