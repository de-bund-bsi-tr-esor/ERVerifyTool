...
/**
 * Enum for timestamp qualities. This enum will currently handle the URIs that end in the given statements. Such URIs might be found inside
 * a SignatureQualityType as defined for the ETSI SignatureValidationReport.
 */
...
public enum TspQuality
{
    ...
    public static TspQuality from(String uri)
    {
        ...
        if (results.size() > 1)
        {
            throw new IllegalArgumentException("Timestamp quality determined from URI "
                + uri
                + " is ambiguous. Following qualities match the uri: "
                + results);
        }
    ...
    /**
     * Qualification status derived from the result. Will be true only for comprehensively checked timestamps from qualified providers
     * according to the official trusted lists
     */
