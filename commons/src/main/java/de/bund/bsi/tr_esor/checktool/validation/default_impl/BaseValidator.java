 ...
 * @author MO
 */
public abstract class BaseValidator<T, C extends ValidationContext<?>, R extends ReportPart> implements Validator<T, C, R>
{
     ...
     * @param prepareValidator optional, do some setup with the validator. The respective type cannot be bound properly, any setup will
     *     require so cast.
     * @param emptyReport creates a report of wanted type (must be exactly the type required as method return value)
     */
    @SuppressWarnings("unchecked")
    protected <RP extends ReportPart, TP> RP callValidator(TP toValidate, Reference ref, Consumer<Validator<?, ?, ?>> prepareValidator,
        Supplier<RP> emptyReport, Class<RP> reportType)
    {
        try
        {
            var val = (Validator<TP, ?, RP>)ValidatorFactory.getInstance().getValidator(toValidate.getClass(), reportType, ctx);
    ...
    /**
     * Convenience for {@link #callValidator(Object, Reference, Consumer, Supplier, Class)} in case of report part has constructor using
     * only {@link Reference} parameter.
     */
    protected <RP extends ReportPart, TP> RP callValidator(TP toValidate, Reference ref, Consumer<Validator<?, ?, ?>> prepareValidator,
        Class<RP> reportType)
    {
    ...
    /**
     * Convenience for {@link #callValidator(Object, Reference, Consumer, Class)} in case no preparation of the validator is necessary.
     */
    ...
    private <RP extends ReportPart> RP supplyReport(Class<RP> type, Reference ref)
    {
        ...
        {
            throw new IllegalArgumentException("no constructor with parameter Reference in type " + type.getName(), e);
        }
        return r;
    }

    /**
     * Returns calculated hash provided by supplier with the given hashOID. If computation fails the result of report is updated according
     * to the occurred exception.
     *
    ...
    protected byte[] computeHash(ByteArraySupplier supplier, String hashOID, Reference ref, ReportPart report)
    {
        ...
        {
            report.updateCodes(ValidationResultMajor.INDETERMINED, "InternalError", MinorPriority.IMPORTANT, e.getMessage(), ref);
        }
        catch (ReflectiveOperationException e)
        {
