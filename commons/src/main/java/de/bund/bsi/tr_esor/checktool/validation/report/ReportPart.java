 ...
 * <p>
 * Implementing classes must define a constructor with single parameter of type {@link Reference}. Furthermore, if other report parts are
 * contained, you might want to define methods of that form:
 *
 * <pre>
 *
...
public class ReportPart
{
     ...
     * {@link #updateCodes(ValidationResultMajor, String, MinorPriority, String, Reference)} replaces the minor code.
     */
    ...
    /**
     * Returns an instance indicating that no validation has been done because of unknown profile. In that case, the application does
     * neither know how to parse data nor how to validate objects.
     */
    ...
    /**
     * Changes result codes and message according to codes produced by some validation. The major code may be changed only to worse values
     * (for instance from indetermined to invalid but not back). The new minor code replaces the existing value if the major code was
     * changed or the new priority is not less important than the last one. Messages are collected, the object is able to use only the
     * message(s) for current level or create a summarized message including all the findings of any sub-validations.
     *
     ...
     */
    public void updateCodes(ValidationResultMajor newMajor, String newMinor, MinorPriority pr, String newMessage, Reference subRef)
    {
        var oldMajor = major;
    ...
    /**
     * Returns <code>true</code> if details are present in the internal data structure (details may be missing if no validation was done due
    ...
    {
        return reference + " {major: " + major + ", minor: " + minor + ", summarizedMessage: " + getSummarizedMessage() + "}";
    }
