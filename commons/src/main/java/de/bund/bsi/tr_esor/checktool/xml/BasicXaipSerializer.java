...
/**
 * XaipSerializer for cases where the Document is not available. Use ComprehensiveXaipSerializer where possible.
 */
public class BasicXaipSerializer implements XaipSerializer
{
    ...
    public byte[] serialize(Object value)
        throws JAXBException, InvalidCanonicalizerException, CanonicalizationException, IOException, IllegalArgumentException
    {
        Element element;
        if (value instanceof VersionManifestType)
        {
            var jaxbElement = new JAXBElement<>(VERSION_MANIFEST_QNAME, VersionManifestType.class, null, (VersionManifestType)value);
        ...
        {
            element = XmlHelper.toElement((CredentialType)value, CONTEXT_PATH, XmlHelper.FACTORY_XAIP::createCredential);
            namespaceMapper.setNSPrefixRecursively(element);
        }
