 ...
 * @author PRE
 */
public abstract class BaseECardSignatureValidator<T, C extends ValidationContext<?>> extends BaseValidator<T, C, SignatureReportPart>
{
    ...
    {
        Element element =
            XmlHelper.toElement(ctx.getReturnVerificationReport(), XmlHelper.FACTORY_OASIS_VR.getClass().getPackage().getName(), null);
    ...
    protected ECard getECard()
    {
        ...
        ((BindingProvider)port).getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, eCardURL.toString());
        return port;
    }
    ...
    {
        var message =
            "No online validation of a potential signature was possible as no validation service is configured in the active profile.";
        result.updateCodes(ValidationResultMajor.INDETERMINED, BsiResultMinor.NOT_SUPPORTED.getUri(), MinorPriority.NORMAL, message, ref);
    ...
    {
        result.updateCodes(ValidationResultMajor.INVALID, BsiResultMinor.HASH_VALUE_MISMATCH.getUri(), MinorPriority.NORMAL, message, ref);
        result.setVr(singleMessageVerificationReport(OasisDssResultMajor.INSUFFICIENT_INFORMATION,
    ...
    {
        result.updateCodes(ValidationResultMajor.INDETERMINED, ECardResultMinor.INTERNAL_ERROR, MinorPriority.NORMAL, message, ref);
        result.setVr(singleMessageVerificationReport(OasisDssResultMajor.RESPONDER_ERROR,
            OasisDssResultMinor.ERROR_RESPONSE_GENERAL_ERROR,
            message));
    }

    private static VerificationReportType singleMessageVerificationReport(OasisDssResultMajor major, OasisDssResultMinor minor,
        String message)
    {
    ...
    private static void fillResultForECardError(SignatureReportPart result, ResponseBaseType response, Reference ref)
    {
    ...
    String generateValidationMessage(ResponseBaseType response, T toCheck)
    {
        ...
        List<JAXBElement<?>> elements =
            any.stream().filter(e -> e instanceof JAXBElement<?>).map(e -> (JAXBElement<?>)e).collect(Collectors.toList());
        if (elements.isEmpty())
        {
            return "Illegal eCard response. Could not parse the existing optional outputs from the eCard response.";
        }

        Optional<JAXBElement<?>> vrElement =
            elements.stream().filter(x -> x.getDeclaredType().equals(VerificationReportType.class)).findAny();
    ...
    static void fillIn(SignatureReportPart report, ResponseBaseType response, String resultMessage)
    {
        if (resultMessage == null)
        {
            var verificationReport = (VerificationReportType)response.getOptionalOutputs()
                ...
                .map(e -> (JAXBElement<?>)e)
                .filter(x -> x.getDeclaredType().equals(VerificationReportType.class))
                .findAny()
