...
public final class ECardResponseUtil
{
    ...
    /**
     * Detects if the eCard result indicates that no signature has been found in the data by the eCard service. Two cases of possible
     * results are known for this case. In the first one, the major result OK is reported but no report is generated. In the other case, a
     * result major error is reported and the message is set to "No Plugin Found". This indicates that a plugin-based signature-detection
     * system has no plugin for the given signature. In case other implementations of the eCard-interface produce different codes if no
    ...
    /**
     * This function decides if an eCard result indicates that a valid report was produced or if a technical error of the eCard service is
     * indicated.
     *
    ...
    static boolean isAcceptableECardResult(ResponseBaseType response)
    {
        ...
        if (resultMinor == null)
        {
            LOG.error("Received non-ok result major {} from eCard service, but no ResultMinor was received.", resultMajor);
            return false;
        }
